...
;
		...
		include "Z80_Params.inc"
		include "HD44780_defs.inc"

;#code   EEPROM_Startup, 0, 30    ;EEPROM mem at 0000h
...
#code   INT_IM1, $38, 30    ;EEPROM mem at 0000h
        jp PIO_A_INT
        align 8
...
IORQ595_SRCK11:			equ $70
IORQ_LS74_E3:			equ $77
HD44780_command			equ $C0
HD44780_data			equ $C1
...
init:   ld a,$03
        call write4bits
        ld b,$90	; ~25cycles/12µs/loop@2MHz-> $90 loops
wait1_init:
                nop
                nop
                djnz wait1_init
        ...
        ld a,$03
        call write4bits
        ld b,$10	; ~25cycles/12µs/loop@2MHz-> $10 loops
wait2_init:
                nop
                nop
                djnz wait2_init
                ...
                ld a,$03
        call write4bits
        ld b,$10	; ~25cycles/12µs/loop@2MHz-> $90 loops
wait3_init:
                nop
                nop
                djnz wait3_init
...
command:             ; a contains value
        ...
        srl a
        srl a
        srl a			;shift to D4-D7
		;// communicate by iorq call to HD44780
		out (HD44780_command),A		;send to command address rs pin =0
...
write:  ld d,$10                ; a contains value
        ...
        srl a			;shift to D4-D7
		;// communicate by iorq call to HD44780
		out (HD44780_data),A		;send to data address rs pin =1
