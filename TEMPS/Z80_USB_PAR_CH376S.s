...
;****************************************************************
USB_TEXT_LABLES		equ 6		; No additional text when using USB.
...
CMD_RET_SUCCESS 	equ $51				; 0101 0001
...
HC376S_CheckConnection::
		...
		ld 		E,$06
		call 	out_CommandByte367S

		ld	 	E,$81					;(hspace+8)			; Testvalue $55 response $AA
		call 	out_DataByte367S
...
HC376S_setUSBMode::
		...
		call	delay20ms    			; 20 msec
		halt	

.omloop1:
		in 		A,(HC376_Command)	; read status bit 4 if busy
		bit 	4,A
		jr 		NZ,.omloop1
...
HC376S_setSDMode::
		...
		ld 		E,$06				; Code used to enable read/write communication and monitoring of the SD card
		call 	out_DataByte367S
...
.omloop2:
		...
		in 		A,(HC376_Data) 		;Check to see if the USB stick is connected or not.
		ld 		E,A
		ld 		D,00
		cp	 	USB_INT_CONNECT				; compare A & USB_INT_CONNECT
		jp		NZ,NoSDpres
	...
	if (USB_TEXT_LABLES>5)
		call 	writeSTRBelow
		db		0,"SD card present! OK.  Code:",0,0
		call 	putDEtoScreen
		call	CRLF
	endif
		xor 	A
...
.loop: 
        ...
        ld      A,E
        out     (HC376_Command),A      ; send actl. byte
...
.loop: 
        ...
        ld      A,E
        out     (HC376_Data),A      ; send actl. byte
